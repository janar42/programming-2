...
class CStudent {
public:
    CStudent(std::string & name, const CDate & born, int enrolled) : m_name(name), m_birth(born), m_enrollYear(enrolled), m_id(idCounter++) {}

    bool operator==(const CStudent & other) const {
        return m_name==other.m_name && m_birth==other.m_birth && m_enrollYear==other.m_enrollYear;
    }
