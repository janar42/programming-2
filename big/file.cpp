...
namespace std { //hash function for storing CPos in unordered set
    template<>
    struct hash<CPos> {
...
public:
    ...
    void setLeft(CTreeNode * left);

    void setRight(CTreeNode * right);
...
CPos::CPos() : m_column(""), m_row(0) {}

CPos::CPos(std::string & str, int n) : m_column(str), m_row(n) {}
...
void CPos::setCol(std::string & col) {
    m_column = col;
}

void CPos::setRow(int row) {
    m_row = row;
}
...
void CDouble::setValue(double dbl) {
    m_value = dbl;
}
...
void CString::setValue(std::string & str) {
    m_value = str;
}
...
void CLeafNode::setValue(CValue val) {
    m_value = val;
}
...
void CAddNode::setLeft(CTreeNode * left) {
    m_left = left;
}

void CAddNode::setRight(CTreeNode * right) {
    m_right = right;
}
...
void CSubNode::setLeft(CTreeNode * left) {
    m_left = left;
}

void CSubNode::setRight(CTreeNode * right) {
    m_right = right;
}
...
void CMulNode::setLeft(CTreeNode * left) {
    m_left = left;
}

void CMulNode::setRight(CTreeNode * right) {
    m_right = right;
}
...
CValue CDivNode::evalNode(std::unordered_set<CPos> &visited) const {
    ...
    if (!bothDouble(leftVal, rightVal)) {
        return {};
    } else if (std::get<double>(rightVal) == 0.0) {
        return {};
    }
    return std::get<double>(leftVal) / std::get<double>(rightVal);
}
...
void CDivNode::setLeft(CTreeNode * left) {
    m_left = left;
}

void CDivNode::setRight(CTreeNode * right) {
    m_right = right;
}
...
void CPowNode::setLeft(CTreeNode * left) {
    m_left = left;
}

void CPowNode::setRight(CTreeNode * right) {
    m_right = right;
}
...
void CEqNode::setLeft(CTreeNode * left) {
    m_left = left;
}

void CEqNode::setRight(CTreeNode * right) {
    m_right = right;
}
...
void CNeqNode::setLeft(CTreeNode * left) {
    m_left = left;
}

void CNeqNode::setRight(CTreeNode * right) {
    m_right = right;
}
...
void CLtNode::setLeft(CTreeNode * left) {
    m_left = left;
}

void CLtNode::setRight(CTreeNode * right) {
    m_right = right;
}
...
void CLeNode::setLeft(CTreeNode * left) {
    m_left = left;
}

void CLeNode::setRight(CTreeNode * right) {
    m_right = right;
}
...
void CGtNode::setLeft(CTreeNode * left) {
    m_left = left;
}

void CGtNode::setRight(CTreeNode * right) {
    m_right = right;
}
...
void CGeNode::setLeft(CTreeNode * left) {
    m_left = left;
}

void CGeNode::setRight(CTreeNode * right) {
    m_right = right;
}
...
void MyExprBuilder::opAdd() {
    ...
    node->setRight(m_stack.top());
    m_stack.pop();
    node->setLeft(m_stack.top());
    m_stack.pop();
    m_stack.push(node);
}
...
bool CSpreadsheet::setCell(CPos pos, std::string contents) {
    ...
    if (std::holds_alternative<double>(value)) {
        CDouble dbl;
        dbl.setValue(std::get<double>(value));
...
void CSpreadsheet::copyRect(CPos dst, CPos src, int w, int h) {
    auto srcCol = src.getCol();
    auto dstCol = dst.getCol();
    int srcX = strToIntCol(srcCol);
    int dstX = strToIntCol(dstCol);
    std::pair<int, int> moveBy = {dstX - srcX, dst.getRow() - src.getRow()};
    std::map<CPos, CData *> tmpMap;

    for (const auto & cell : m_sheet) {
        if (cell.first.getCol() >= src.getCol() && cell.first.getCol() < addToCol(srcCol, w) && cell.first.getRow() >= src.getRow() && cell.first.getRow() < src.getRow() + h) {
            auto sourceStr = cell.first.getCol();
            auto col = addToCol(sourceStr, moveBy.first);
            CPos newPos(col, cell.first.getRow() + moveBy.second);
